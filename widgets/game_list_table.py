...
class GameListTableModel(QtCore.QAbstractTableModel):
    ...
    def _load_cache_slice(self, center_row):
        ...
        print(
            f"[GameListTableModel] Lazy loading chunk: rows {start} to {start + len(rows)} (requested row: {center_row})"
        )
...
class ColumnConfigDialog(QtWidgets.QDialog):
    def __init__(self, header_view, headers, parent=None):
        ...
        # Dynamic color for label depending on dark mode
        dark = False
        ...
        if app:
            for w in app.topLevelWidgets():
                if w.__class__.__name__ == "ChessCQLiApp" and hasattr(w, "dark_mode"):
                    dark = w.dark_mode
                    break
        color = "#9CA3AF" if dark else "#666666"

        info_lbl = QtWidgets.QLabel(
            "Drag items or use buttons to reorder.\nCheck/uncheck to show/hide columns."
        )
        info_lbl.setStyleSheet(f"color: {color}; font-size: 11px;")
