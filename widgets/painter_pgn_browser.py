import re
from PyQt5.QtCore import Qt, QRect, pyqtSignal, QUrl, QSettings
from PyQt5.QtGui import QColor, QFont, QPainter, QFontMetrics, QCursor
from PyQt5.QtWidgets import QScrollArea, QWidget, QMenu, QAction, QVBoxLayout
import chess.pgn

from widgets.pgn_browser import CommentDialog
from dialogs.pgn_browser_config_dlg import PGNBrowserConfigDialog
...
class QPainterBrowser(QWidget):
    ...
    def paintEvent(self, event):
        ...
        viewport_bottom = (
            viewport_top + self.parent_browser.scroll_area.viewport().height()
        )
        ...
        for block in self.blocks:
            if hasattr(block, "rect") and block.rect:
                if (
                    block.rect.bottom() < viewport_top - 50
                    or block.rect.top() > viewport_bottom + 50
                ):
                    continue
            ...
            for token in block.tokens:
                ...
                if (
                    token.rect.bottom() < viewport_top
                    or token.rect.top() > viewport_bottom
                ):
                    continue
                ...
                if token.token_type == "move":
                    ...
                    show_classifications = getattr(
                        self.parent_browser, "show_classifications", True
                    )
                    if (
                        show_classifications
                        and hasattr(token, "classification")
                        and token.classification in cls_colors
                    ):
                        painter.setPen(cls_colors[token.classification])
                    else:
                        painter.setPen(
                            main_move_color if token.level == 0 else var_move_color
                        )

                    painter.drawText(
                        token.rect.x() + 3,
                        token.rect.y() + fm_normal.ascent(),
                        token.text,
                    )
    ...
    def mousePressEvent(self, event):
        ...
        for block in self.blocks:
            for token in block.tokens:
                if (
                    token.token_type == "move"
                    and token.rect
                    and token.rect.contains(pos)
                ):
...
class QPainterHeaderWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        headers = {}
        if (
            hasattr(self.parent_browser.move_manager, "game")
            and self.parent_browser.move_manager.game
        ):
        ...
        is_new_game = (
            white == "?"
            and black == "?"
            and (event_name == "?" or event_name == "Chess Analysis" or not event_name)
        )
        ...
        if is_new_game:
            ...
            sub_text_elided = fm_sub.elidedText(
                sub_text, Qt.ElideRight, available_width
            )
...
class QPainterPGNBrowser(QWidget):
    ...
    def __init__(self, parent, move_manager):
        ...
        self.scroll_area.verticalScrollBar().valueChanged.connect(
            lambda _: self.paint_widget.update()
        )
        self.main_layout.addWidget(self.scroll_area)

        self.setContextMenuPolicy(Qt.CustomContextMenu)
        self.customContextMenuRequested.connect(self.on_custom_context)
        self.paint_widget.setContextMenuPolicy(Qt.CustomContextMenu)
        self.paint_widget.customContextMenuRequested.connect(
            self.on_paint_widget_context
        )
    ...
    def setStyleSheet(self, stylesheet: str):
        ...
        if font_match:
            self.font_family = (
                font_match.group(1).replace("'", "").replace('"', "").strip()
            )
    ...
    def rebuild_layout(self, force=False):
        ...
        nodes_changed = True
        if hasattr(self, "cached_nodes") and len(self.cached_nodes) == len(
            current_nodes
        ):
            nodes_changed = False
        ...
        width_changed = (
            not hasattr(self, "last_layout_width")
            or self.last_layout_width != layout_width
        )
        ...
        if (
            ...
            or nodes_changed
            or width_changed
            or getattr(self, "layout_invalid", False)
        ):
    ...
    def traverse_layout_a(
        self, node, level, blocks, flat_nodes, show_comments, show_variations
    ):
        curr = node
        while curr is not None:
            if (
                hasattr(curr, "san")
                and hasattr(curr, "move_number")
                and hasattr(curr, "turn")
            ):
                san = curr.san
            ...
            if is_white_turn:
                ...
                if eval_text:
                    block.tokens.append(
                        Token("eval", eval_text, move_idx=move_idx, level=level)
                    )
                blocks.append(block)
            else:
                if (
                    blocks
                    and blocks[-1].tokens
                    and blocks[-1].tokens[-1].token_type == "move"
                    and blocks[-1].level == level
                    and "..." not in blocks[-1].tokens[0].text
                ):
                    blocks[-1].tokens.append(
                        Token(
                            "move",
                            san,
                            move_idx=move_idx,
    ...
    def traverse_layout_b(
        ...
        while curr is not None:
            ...
            if is_white_turn:
                if need_prefix:
                    current_block.tokens.append(
                        Token("num", f"{move_num}.", level=level)
                    )
                    need_prefix = False
            ...
            current_block.tokens.append(
                Token(
                    "move", san, move_idx=move_idx, level=level, classification=cls_val
                )
            )
    ...
    def load_settings(self):
        settings = QSettings("TestChessApp", "PGNBrowser")
        self.layout_mode = int(settings.value("layout_mode", 1))
        self.show_eval = settings.value("show_eval", True, type=bool)
        self.show_classifications = settings.value(
            "show_classifications", True, type=bool
        )
        self.show_comments = settings.value("show_comments", True, type=bool)
        self.show_variations = settings.value("show_variations", True, type=bool)
        self.cb_compact = settings.value("cb_compact", False, type=bool)

    def save_settings(self):
        settings = QSettings("TestChessApp", "PGNBrowser")
        settings.setValue("layout_mode", self.layout_mode)
        settings.setValue("show_eval", self.show_eval)
        settings.setValue("show_classifications", self.show_classifications)
        settings.setValue("show_comments", self.show_comments)
        settings.setValue("show_variations", self.show_variations)
        settings.setValue("cb_compact", self.cb_compact)
    ...
    def show_config_dialog(self):
        current_cfg = {
            ...
            "cb_compact": self.cb_compact,
        }
        dlg = PGNBrowserConfigDialog(self, initial_settings=current_cfg)
        if dlg.exec_() == PGNBrowserConfigDialog.Accepted:
            new_cfg = dlg.get_settings()
            self.layout_mode = new_cfg["layout_mode"]
            self.show_eval = new_cfg["show_eval"]
            self.show_classifications = new_cfg["show_classifications"]
            self.show_comments = new_cfg["show_comments"]
            self.show_variations = new_cfg["show_variations"]
            self.cb_compact = new_cfg["cb_compact"]
            self.save_settings()
            self.rebuild_layout(force=True)

    def on_paint_widget_context(self, point):
        self._show_context_menu(point, is_paint_widget=True)

    def on_custom_context(self, point):
        self._show_context_menu(point, is_paint_widget=False)

    def _show_context_menu(self, point, is_paint_widget=False):
        paint_point = (
            point if is_paint_widget else self.paint_widget.mapFrom(self, point)
        )
        ...
        for block in self.blocks:
            for token in block.tokens:
                if (
                    token.token_type == "move"
                    and token.rect
                    and token.rect.contains(paint_point)
                ):
        ...
        config_act = QAction(
            qta.icon("fa5s.sliders-h"), "Configure PGN Browser...", self
        )
